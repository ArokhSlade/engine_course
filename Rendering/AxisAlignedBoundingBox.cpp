...
void AxisAlignedBoundingBox::CalculateDimensions()
{
	...
	m_posX = (m_maxX + m_minX) / 2;
	m_posY = (m_maxY + m_minY) / 2;
	m_posZ = (m_maxZ + m_minZ) / 2;
}
...
bool AxisAlignedBoundingBox::BuildLineBuffers(ID3D11Device* device)
{
	...
	XMFLOAT4 colorGreen = XMFLOAT4(0, 1, 0, 1);
	...
	// Bottom
	{
		...
		vertices[index].position = DirectX::XMFLOAT3(m_maxX, m_minY, m_minZ);
		vertices[index].color = colorGreen;
		indices[index] = index;
		index++;

		vertices[index].position = DirectX::XMFLOAT3(m_maxX, m_minY, m_maxZ);
		vertices[index].color = colorGreen;
		indices[index] = index;
		index++;
		...
		vertices[index].position = DirectX::XMFLOAT3(m_minX, m_minY, m_maxZ);
		vertices[index].color = colorGreen;
		indices[index] = index;
		index++;
		...
		vertices[index].position = DirectX::XMFLOAT3(m_minX, m_minY, m_minZ);
		vertices[index].color = colorGreen;
		indices[index] = index;
		index++;
	}
