...
bool SkyDomeShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	bool result = InitializeShader( device, hwnd, 
									L"../Rendering/data/Skydome/skydome.vs", 
									L"../Rendering/data/Skydome/skydome.ps");
	if (!result)
	{
		return false;
	}
...
bool SkyDomeShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, 
...
bool SkyDomeShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilePath, WCHAR* psFilePath)
{
	HRESULT result;
	...
	//name can be found inside skydome.vs
	result = D3DCompileFromFile(vsFilePath, NULL, NULL, "SkyDomeVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	if (FAILED(result))
	{
		...
		{
			MessageBox(hwnd, vsFilePath, L"Missing SkyDome Shader File", MB_OK);
		}
	...
	//name can be found inside skydome.ps
	result = D3DCompileFromFile(psFilePath, NULL, NULL, "SkyDomePixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	if (FAILED(result))
	{
		...
		{
			MessageBox(hwnd, psFilePath, L"Missing SkyDome Shader File", MB_OK);
		}
...
void SkyDomeShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFileName)
{
	...
	fout.open("shader-error.txt"); //will not overwrite during runtime, but will be cleared on new start
...
bool SkyDomeShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, XMFLOAT4 apexColor, XMFLOAT4 centerColor)
{
	HRESULT result;
...
//copied without change from ColorShaderClass
