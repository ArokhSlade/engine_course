...
bool SkyDome::Initialize(ID3D11Device* device)
{
	...
	m_apexColor = XMFLOAT4(0.f, 0.5f, 0.6f, 1.f); //format comes from directx rgba
...
void SkyDome::RenderBuffers(ID3D11DeviceContext* deviceContext) ////copied from ModelClass, renamed device to deviceContext?
{
	unsigned int stride;
	unsigned int offset;
...
bool SkyDome::LoadModel(char* filename) //copied from ModelClass
{
	ifstream fin;
	char input;
	int i;
	...
	m_model = new ModelType[m_vertexCount]; //what's inside text file pos, texture, normal	
	if (!m_model)
	{
		return false;
	}
